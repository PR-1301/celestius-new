...
export default function Login({ setActivePage }) {
  ...
  // If already signed in, immediately forward to Dashboard
  useEffect(() => {
    if (isSignedIn) {
  ...
  // Restore any persisted error (e.g. from Google OAuth rejection redirect)
  useEffect(() => {
    try {
      const persistedError = sessionStorage.getItem('celestius_auth_error');
      if (persistedError) {
        setAuthError(persistedError);
        sessionStorage.removeItem('celestius_auth_error');
      }
    } catch {
      // ignore
    }
  ...
  }, [mode, setAuthError, setVerificationPending]);

  const validateCitDomain = (rawEmail) => {
    if (!rawEmail || !rawEmail.toLowerCase().trim().endsWith('@citchennai.net')) {
      const err = 'Access Denied: Only @citchennai.net university accounts are permitted.';
      setAuthError(err);
      return false;
    }
    return true;
  ...
  // If already signed in, prevent rendering login card
  if (isSignedIn) {
    return (
      <div className="min-h-screen flex items-center justify-center font-mono text-xs text-zinc-400">
        <Loader2 className="w-5 h-5 animate-spin text-[#FFCC00] mr-2" />
      ...
      <div className="w-full max-w-5xl mx-auto z-10">
        ...
        <div className="mb-6 flex items-center justify-between">
          <button
            ...
            className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-black/60 hover:bg-black/90 border border-white/10 hover:border-white/30 text-zinc-400 hover:text-white font-mono text-xs transition-all duration-200 cursor-pointer"
          >
          ...
          <div className="lg:col-span-7 p-6 sm:p-10 rounded-3xl bg-[#0a0a0e] border border-white/15 shadow-2xl relative">
            ...
            {actionSuccess && (
              <div className="absolute inset-0 bg-[#0a0a0e]/95 backdrop-blur-md rounded-3xl flex flex-col items-center justify-center p-6 text-center z-20 animate-fade-in">
                ...
                <p className="font-mono text-xs text-zinc-400 mt-1">Initializing user session and redirecting to Dashboard...</p>
              </div>
            )}
              ...
              {!verificationPending && (
                <div className="grid grid-cols-2 p-1 bg-black/70 rounded-2xl border border-white/10 font-mono text-xs">
                  <button
                    ...
                    className={`py-3 rounded-xl font-bold transition-all tracking-wider cursor-pointer ${
                      isSignIn
              ...
              {!verificationPending && (
                <div>
                  <button
                    ...
                    className="w-full flex items-center justify-center gap-3 py-3.5 px-4 rounded-2xl bg-zinc-900/90 hover:bg-zinc-800 border border-white/20 hover:border-white/40 text-white font-mono text-xs font-semibold tracking-wider transition-all duration-200 active:scale-[0.99] disabled:opacity-70 disabled:cursor-not-allowed shadow-sm cursor-pointer"
                  >
                    {googleConnecting ? (
                      <>
                        <Loader2 className="w-4 h-4 animate-spin text-zinc-300" />
                        <span className="text-zinc-200">Connecting to Google...</span>
                      </>
                    ) : (
                      <>
                  ...
                  <p className="font-mono text-[10px] text-zinc-400 text-center mt-2 flex items-center justify-center gap-1.5">
                    <Shield className="w-3 h-3 text-[#FFCC00]" />
                    <span>Only official <strong className="text-[#FFCC00]">@citchennai.net</strong> accounts authorized.</span>
                  </p>
                ...
                {verificationPending ? (
                  ...
                  <div className="space-y-4">
                    ...
                    <button
                      ...
                      className="w-full flex items-center justify-center gap-2 py-3.5 rounded-2xl bg-[#FFCC00] hover:bg-[#FFE066] text-black font-mono text-xs font-bold tracking-wider transition-all disabled:opacity-50 shadow-md shadow-[#FFCC00]/15 cursor-pointer"
                    >
                      {authLoading ? (
                    ...
                    <button
                      ...
                      className="w-full text-center font-mono text-xs text-zinc-500 hover:text-white transition-colors cursor-pointer"
                    >
                  ...
                  <>
                    ...
                    <div className="space-y-1.5">
                      <div className="flex items-center justify-between">
                        ...
                        <span className="font-mono text-[9px] text-[#FFCC00] flex items-center gap-1 font-semibold">
                          <Shield className="w-2.5 h-2.5 text-[#FFCC00]" />
                          REQUIRED: @citchennai.net
                        </span>
                      </div>
                      <input
                        type="email"
                      ...
                      <p className="font-mono text-[10px] text-zinc-500">
                        Personal emails (@gmail.com, etc.) are strictly rejected by the university firewall.
                      </p>
                    </div>
                    ...
                    <div className="pt-2">
                      <button
                        type="submit"
                        disabled={authLoading}
                        className="w-full flex items-center justify-center gap-2 py-3.5 rounded-2xl bg-[#FFCC00] hover:bg-[#FFE066] active:scale-[0.99] text-black font-mono text-xs font-bold tracking-wider transition-all shadow-md shadow-[#FFCC00]/20 disabled:opacity-50 cursor-pointer"
                      >
                        {authLoading ? (
                          <Loader2 className="w-4 h-4 animate-spin text-black" />
                        ) : (
                  ...
                  {isSignIn ? (
                    <span>
                      ...
                      <button
                        ...
                        className="text-[#FFCC00] hover:underline font-semibold cursor-pointer"
                      >
                        Register here
                      </button>
                    </span>
